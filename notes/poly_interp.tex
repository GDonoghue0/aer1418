...
\begin{proposition}[polynomial interpolation error bound]
  ...
  Let $w \in H^{s+1}(\Omega)$, $\{\calT_h\}_{h > 0}$ be a family of shape-regular triangulations, and $\calI_h w$ be the piecewise polynomial interpolant of degree $p$ associated with $\calT_h$. If $w \in H^{s+1}(\Omega)$, then % the $L^2(\Omega)$ interpolation error is bounded by
\begin{align*}
