...
\section{Variational form (or weak form)}
\label{sec:pos1d_var}
We now consider a different form of~\eqref{eq:pos1d_strong} that is amenable to finite element discretization. To begin, we choose a (sufficiently regular) test function such that $v(x=0) = v(x=1) = 0$, multiply~\eqref{eq:pos1d_strong} by the function, and integrate the expression to obtain
\begin{equation}
  \int_\Omega v \left( -\dd{^2u}{x^2} \right) dx = \int_\Omega v f dx.
  \label{eq:pos1d_weak_pre}
\end{equation}
We then integrate by parts the left hand side to obtain
\begin{equation*}
  \int_\Omega \dd{v}{x} \dd{u}{x} dx = \int_\Omega v f dx;
  \label{eq:pos1d_weak}
\end{equation*}
note that the boundary terms vanish because we require $v(x=0) = v(x=1) = 0$. If the $u$ is the solution to~\eqref{eq:pos1d_strong}, then we expect the expression~\eqref{eq:pos1d_weak_pre}, and in turn~\eqref{eq:pos1d_weak}, to hold for \emph{any} test function $v$.  In fact, the \emph{variational form}, which is also known as the \emph{weak form}, is derived from seeking the solution $u$ which satisfies~\eqref{eq:pos1d_weak_pre} for all suitable test functions $v$.  We may speculate that, given we consider a suitably large set of test functions, the solution $u$ is unique and coincide with the solution to~\eqref{eq:pos1d_strong}.

We now formalize the procedure. To this end, we first introduce a linear space
\begin{equation}
  \calV \equiv \{ v \ | \
   v \text{ is continuous, } 
  ...
  \label{eq:pos1d_V}
\end{equation}
The first two conditions are related to the smoothness of the solution sought, and the last condition imposes the boundary condition. We then introduce a \emph{bilinear form} $a: \calV \times \calV \to \RR$,
\begin{equation}
...
\end{align*}
Our \emph{variational form} of the problem is as follows: find $u \in \calV$ such that
\begin{equation}
...
We can readily show that the solution to the strong from~\eqref{eq:pos1d_strong} solves the variational form~\eqref{eq:pos1d_var}.  To see this, we observe that, for all $v \in \calV$,
\begin{equation*}
  a(u,v) - \ell(v) \equiv
...
\end{equation*}
the boundary terms again vanish because $v(x=0) = v(x=1) = 0$ for all $v \in \calV$. Hence, the solution to the strong form~\eqref{eq:pos1d_strong} satisfies the weak form~\eqref{eq:pos1d_var}. 

While the solution to the strong form~\eqref{eq:pos1d_strong} is also the solution to the variational form~\eqref{eq:pos1d_var}, the converse is not true in general.  In fact, the variational form admits more general loads $f$ and associated solutions than the strong form.  For instance, a Dirac delta function, which corresponds to a point load, is an admissible load for the \eqref{eq:pos1d_var} and there exists a unique solution to the problem; however, the strong from~\eqref{eq:pos1d_strong} is not well defined for a Dirac delta function $f$ because $f$ does not have well-defined point-wise values. 

\section{Minimization form}
We now introduce a \emph{minimization form}, which is closely related to the variational form~\eqref{eq:pos1d_var}. We note that not all boundary value problems possess a minimization form; only those problems with an intrinsic energy, such as our model problem~\eqref{eq:pos1d_strong}, possesses a minimization form. To obtain a minimization form, we introduce a functional $J: \calV \to \RR$ given by
\begin{equation}
  J(w) \equiv \frac{1}{2} \int_\Omega \left( \dd{w}{x} \right)^2 dx - \int_\Omega f w dx \quad \forall w \in \calV,
  \label{eq:pos1d_J}
\end{equation}
where $\calV$ is defined in~\eqref{eq:pos1d_V}. Our \emph{minimization problem} is as follows: find $u \in \calV$ such that
\begin{equation}
...
\end{equation*}
which the exact condition of our variational problem~\eqref{eq:pos1d_var}. Hence we conclude that $u \in \calV$ is the solution to~\eqref{eq:pos1d_var} if and only if it is the solution to~\eqref{eq:pos1d_min}; the two problem are equivalent.  It follows that, as noted in  Section~\ref{sec:pos1d_var}, if the solution $u$ solves the strong form~\eqref{eq:pos1d_strong}, then it also the solution to the minimization form~\eqref{eq:pos1d_min}; however, the minimization form admits more general loads $f$ and associated solutions than the strong form. We will soon see that we can derive a finite element approximation from either~\eqref{eq:pos1d_var} or \eqref{eq:pos1d_min}.
...
We can now state our FE approximation problem in either the variational form or the minimization form. The FE variational form is as follows: find $u_h \in \calV_h$ such that
\begin{equation}
  a(u_h,v) = \ell(v) \quad \forall v \in \calV_h,
  \label{eq:pos1d_var_fe}
\end{equation}
where $a: \calV \times \calV \to \RR$ and $\ell: \calV \to \RR$ are bilinear form~\eqref{eq:pos1d_a} and linear form~\eqref{eq:pos1d_ell}, respectively. In words, we seek the solution in and test against the finite-dimensional (hence computable) subspace $\calV_h$ of $\calV$.  Similarly, the FE minimization form is as follows: find $u_h \in \calV_h$ such that
\begin{equation}
  u_h = \argmin_{v \in \calV_h} J(v),
  \label{eq:pos1d_min_fe}
\end{equation}
where $J: \calV \to \RR$ is the energy functional~\eqref{eq:pos1d_J}. In words, we seek the minimizer in the subspace $\calV_h$ of $\calV$.  %Note that $v \in \calV_h$ is an admissible argument of $J$ since $\calV_h \subset \calV$.
