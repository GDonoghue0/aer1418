...
To obtain a minimization form of~\eqref{eq:gen_inhomo_bc_strong}, we introduce a spaces
\begin{align*}
...
We now show that the solution to~\eqref{eq:gen_inhomo_bc_strong} satisfies the minimization statement~\eqref{eq:gen_inhomo_bc_min}. We first set $w = u + v$, where $u \in \calV_D$ is the solution to~\eqref{eq:gen_inhomo_bc_strong} and $v$ is an arbitrary function in $\calV$ (and \emph{not} $\calV_D$). We then observe
\begin{align*}
  J(u + v)
  &=
